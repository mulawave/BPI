...
export default function StoreShell({ session, children }: StoreShellProps) {
          ...
          <div className="flex items-center justify-between h-16">
            <Link href="/dashboard" onClick={() => handleNavClick("/dashboard")} className="flex items-center gap-2.5 shrink-0">
              ...
              <div className="hidden sm:block">
                ...
                <span className="ml-1.5 text-[10px] uppercase tracking-[0.2em] font-semibold text-emerald-600 dark:text-emerald-400">Store</span>
              </div>
            </Link>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={openCart}
                className="relative flex items-center justify-center h-9 w-9 rounded-lg text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
                aria-label="Open cart"
              >
                <ShoppingBag className="w-4 h-4" />
                {totalItems > 0 && (
                  <span className="absolute -right-0.5 -top-0.5 flex h-4 min-w-[1rem] items-center justify-center rounded-full bg-emerald-600 px-1 text-[9px] font-bold text-white">

...
export function CspDashboard({ userName }: CspDashboardProps) {
  ...
  return (
    <div className="space-y-5 pb-4">
        ...
        <div className="relative px-6 py-6 sm:px-8 sm:py-8">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            <div className="flex-1">
              ...
              {/* Countdown Timer */}
              <div className="mt-5">
                <p className="text-[10px] uppercase tracking-[0.3em] font-semibold text-amber-200/70 mb-2.5">
                  {countdown.expired ? "CSP Season is Live" : "CSP Season Launch"}
                </p>
                {countdown.expired ? (
                  <div className="inline-flex items-center gap-2 rounded-xl bg-emerald-500/15 backdrop-blur border border-emerald-300/30 px-5 py-3">
                    <span className="h-2 w-2 rounded-full bg-emerald-400 animate-pulse" />
                    <span className="font-serif text-lg font-bold text-emerald-200">Live Now</span>
                  </div>
                ) : (
                    ...
                    {[
                      ...
                      <div key={unit.label} className="flex items-center gap-2 sm:gap-3">
                        <div className="relative group">
                          <div className="absolute -inset-0.5 rounded-lg bg-gradient-to-br from-amber-300/20 to-emerald-400/10 blur-sm group-hover:from-amber-300/30 group-hover:to-emerald-400/20 transition-all" />
                          <div className="relative rounded-lg bg-white/[0.08] backdrop-blur-xl border border-amber-200/20 px-3 py-2 sm:px-4 sm:py-2.5 min-w-[56px] sm:min-w-[68px] text-center">
                            <p className="font-serif text-xl sm:text-2xl font-bold text-white tabular-nums tracking-tight">
                              {String(unit.value).padStart(2, "0")}
                            </p>
                            <p className="text-[8px] sm:text-[9px] uppercase tracking-[0.15em] font-semibold text-amber-200/60 mt-0.5">
                              {unit.label}
                            </p>
                          </div>
                        </div>
                        {idx < arr.length - 1 && (
                          <span className="font-serif text-xl sm:text-2xl font-bold text-amber-300/40">:</span>
                        )}
                      </div>
                    ))}
                  </div>
      ...
      {tierProfile ? (
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-white via-white to-slate-50 dark:from-slate-950 dark:via-slate-950 dark:to-emerald-950/30 border border-slate-200/70 dark:border-slate-800/70 shadow-lg h-full">
        ...
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-50 to-white dark:from-slate-900/40 dark:via-slate-950 dark:to-emerald-950/10 border border-dashed border-slate-300 dark:border-slate-700 shadow-sm h-full">
          <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-amber-400/30 to-transparent" />
          <div className="relative p-5 flex items-center gap-3 h-full">
      ...
      {(waitStatusQuery.data?.hasCooldown || waitStatusQuery.data?.sponsorProgress) ? (
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-50/80 to-white dark:from-amber-950/20 dark:via-slate-950 dark:to-amber-950/10 border border-amber-300/50 dark:border-amber-700/40 shadow-lg h-full">
        ...
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-50 to-white dark:from-slate-900/40 dark:via-slate-950 dark:to-amber-950/10 border border-dashed border-slate-300 dark:border-slate-700 shadow-sm h-full">
          <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-amber-400/30 to-transparent" />
          <div className="relative p-5 flex items-center gap-3 h-full">
      ...
      {recognitionQuery.data ? (
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-50/70 to-white dark:from-emerald-950/20 dark:via-slate-950 dark:to-amber-950/10 border border-emerald-200/60 dark:border-emerald-800/40 shadow-lg h-full">
      ...
      <div className="grid lg:grid-cols-3 gap-4 items-stretch">
        {/* Request form */}
        <div className="lg:col-span-2 relative overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-900 via-emerald-950 to-slate-950 border border-emerald-700/40 shadow-lg">
          <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-amber-400/60 to-transparent" />
          <div className="absolute -top-20 -right-20 w-48 h-48 rounded-full bg-amber-400/8 blur-3xl" />

          <div className="relative p-5 space-y-4">
            <div className="flex items-start justify-between gap-2">
              <div className="flex items-start gap-2.5">
                <div className="flex items-center justify-center h-10 w-10 rounded-lg bg-gradient-to-br from-amber-300 to-amber-600 shadow-md ring-1 ring-amber-200/50">
                  <ScrollText className="w-4 h-4 text-white" />
                </div>
                <div>
                  <p className="text-[9px] uppercase tracking-[0.25em] font-semibold text-amber-200/80">Petition of Support</p>
                  <h3 className="font-serif text-xl font-bold text-white">Submit a Request</h3>
                </div>
              </div>
              <span className={"inline-flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-semibold rounded-full ring-1 " + (eligibility.eligible ? "bg-emerald-500/15 text-emerald-200 ring-emerald-300/40" : "bg-amber-500/15 text-amber-200 ring-amber-300/40")}>
            ...
            {/* Category selector */}
            <div>
              <label className="text-[10px] uppercase tracking-[0.25em] font-semibold text-amber-200/70">Support Category</label>
                ...
                {(["national", "global"] as SupportCategory[]).map((cat) => {
                  ...
                  return (
                    <button
                      ...
                      className={"group relative overflow-hidden rounded-xl border-2 px-4 py-3 text-left transition-all " + (active ? "border-emerald-400 bg-white/[0.08] backdrop-blur shadow-lg" : "border-white/10 hover:border-emerald-400/40 bg-white/[0.03]")}
                    >
                      {active && <div className="absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-emerald-400 to-amber-400" />}
                      <div className="flex items-center justify-between">
                        <span className="font-serif text-base font-bold text-white">{rules?.label ?? cat}</span>
                        <Globe className={`w-4 h-4 ${active ? "text-emerald-300" : "text-emerald-200/40"}`} />
                      </div>
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              <div>
                <label className="text-[10px] uppercase tracking-[0.25em] font-semibold text-amber-200/70">Requested Amount</label>
                <input
                  type="number"
                  value={amount}
                  onChange={(e) => setAmount(e.target.value)}
                  className="mt-2 w-full rounded-xl border border-emerald-700/40 bg-white/[0.06] backdrop-blur px-4 py-3 text-base font-serif font-bold text-white focus:border-emerald-400 focus:ring-2 focus:ring-emerald-400/20 outline-none transition placeholder:text-emerald-200/30"
                ...
                {amount && !Number.isNaN(Number(amount)) && Number(amount) > 0 && (
                  <p className="mt-1.5 text-[11px] text-emerald-100/50">
                    Broadcast target: <span className="font-semibold text-amber-200">{formatAmount(Math.ceil(Number(amount) * 1.2))}</span> <span className="text-amber-300/70">(+20% markup)</span>
                  </p>
                )}
              </div>
              <div>
                <label className="text-[10px] uppercase tracking-[0.25em] font-semibold text-amber-200/70">Purpose</label>
                <input
                  type="text"
                  value={purpose}
                  onChange={(e) => setPurpose(e.target.value)}
                  className="mt-2 w-full rounded-xl border border-emerald-700/40 bg-white/[0.06] backdrop-blur px-4 py-3 text-sm text-white focus:border-emerald-400 focus:ring-2 focus:ring-emerald-400/20 outline-none transition placeholder:text-emerald-200/30"
            ...
            <div>
              <label className="text-[10px] uppercase tracking-[0.25em] font-semibold text-amber-200/70">Additional Details</label>
              <textarea
                value={notes}
                onChange={(e) => setNotes(e.target.value)}
                rows={3}
                className="mt-2 w-full rounded-xl border border-emerald-700/40 bg-white/[0.06] backdrop-blur px-4 py-3 text-sm text-white focus:border-emerald-400 focus:ring-2 focus:ring-emerald-400/20 outline-none transition resize-none placeholder:text-emerald-200/30"
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <span key={i} className={"inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-[11px] font-semibold ring-1 " + (s.ok ? "bg-emerald-500/15 text-emerald-200 ring-emerald-300/40" : "bg-amber-500/15 text-amber-200 ring-amber-300/40")}>
                  {s.ok ? <CheckCircle2 className="h-3 w-3" /> : <AlertTriangle className="h-3 w-3" />}
                  {s.ok ? s.on : s.off}
                </span>
              ))}
              {supportCategory === "global" && eligibility.globalPath && (
                <span className="inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-[11px] font-semibold ring-1 bg-blue-500/15 text-blue-200 ring-blue-300/40">
            ...
            {/* Submit */}
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-2 border-t border-emerald-800/40">
              <div className="text-[11px] text-emerald-100/50 flex items-center gap-1.5">
              ...
              <button
                ...
                className="group relative inline-flex items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-amber-400 to-amber-600 px-5 py-2.5 text-xs font-semibold uppercase tracking-wider text-emerald-950 shadow-md ring-1 ring-amber-300/50 hover:shadow-lg hover:from-amber-300 hover:to-amber-500 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
              >
        ...
        {/* Live status */}
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-900 via-emerald-950 to-slate-950 border border-emerald-700/40 shadow-lg h-full">
          <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-amber-400/60 to-transparent" />

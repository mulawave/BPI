...
export default function CspShell({ session, children }: CspShellProps) {
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-white via-white to-emerald-50/40 dark:from-slate-950 dark:via-emerald-950/40 dark:to-slate-950 transition-colors duration-500">
      <header className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? "bg-white/90 dark:bg-slate-950/90 backdrop-blur-xl shadow-xl dark:shadow-emerald-950/50" : "bg-white/60 dark:bg-slate-950/60 backdrop-blur-md shadow-lg dark:shadow-emerald-950/30"} border-b border-slate-200/60 dark:border-emerald-800/40`}>
        <div className="mx-auto max-w-[1600px] px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <Link href="/dashboard" onClick={(e) => handleNavClick("/dashboard", e)} className="flex items-center gap-2.5 shrink-0">
                    ...
                    <div className="p-4 space-y-3">
                      <div className="flex items-center gap-3">
                        <div className="relative h-12 w-12 rounded-full bg-gradient-to-br from-emerald-500 to-emerald-700 flex items-center justify-center ring-2 ring-amber-300/30 shadow-md shrink-0">
                          {profile?.image ? <img src={profile.image} alt="" className="h-12 w-12 rounded-full object-cover" /> : <User className="w-5 h-5 text-white" />}
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div className="rounded-lg border border-slate-200 dark:border-slate-800 p-2.5">
                          ...
                          <p className="text-xs font-bold text-slate-900 dark:text-white font-mono">{profile?.walletBalance != null ? formatAmountShort(profile.walletBalance) : "—"}</p>
                        </div>
                        <div className="rounded-lg border border-slate-200 dark:border-slate-800 p-2.5">
                          <div className="flex items-center gap-1.5 mb-1">
                          ...
                          <p className="text-xs font-bold text-slate-900 dark:text-white">{profile?.cspTierName || "—"}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 pt-1 border-t border-slate-100 dark:border-slate-800">
                        <Link href="/settings" onClick={(e) => { setProfileOpen(false); handleNavClick("/settings", e); }} className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-lg border border-slate-200 dark:border-slate-800 px-3 py-2 text-xs font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">

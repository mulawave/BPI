...
export default function UnauthBlogArticle({ slug: slugProp }: { slug?: string }) {
    ...
    <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 space-y-8">
      ...
      <article className="overflow-hidden rounded-3xl border border-border bg-background/90 shadow-xl">
        <div className="relative h-[260px] sm:h-[420px] w-full">
          <img src={post.image || post.imageUrl || "/img/blog-placeholder.jpg"} alt={post.title} className="h-full w-full object-cover" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent" />
          <div className="absolute bottom-6 left-6 right-6 text-white">
            <div className="flex flex-wrap items-center gap-3 text-xs sm:text-sm">
              {post.category && <span className="rounded-full bg-white/20 px-3 py-1 font-semibold backdrop-blur">{post.category.name}</span>}
        ...
        {commentsQuery.data && commentsQuery.data.totalPages > 1 && (
          <div className="mt-4 flex items-center justify-between text-sm text-muted-foreground">
            ...
            <div className="flex items-center gap-2">
              <button disabled={commentsPage <= 1} onClick={() => setCommentsPage((p) => Math.max(1, p - 1))} className="rounded-lg border border-border px-3 py-1 disabled:opacity-50">Prev</button>
              <button disabled={commentsPage >= (commentsQuery.data?.totalPages || 1)} onClick={() => setCommentsPage((p) => p + 1)} className="rounded-lg border border-border px-3 py-1 disabled:opacity-50">Next</button>
            </div>
          </div>
        )}
      </section>

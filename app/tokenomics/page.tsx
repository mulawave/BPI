      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="p-6 border rounded-lg">
          ...
          <p className="text-3xl font-bold">
            {data?.buyBackWalletBalance.toLocaleString() ?? 0}
          </p>
          <p className="text-sm text-gray-500">Value (NGN)</p>
        </div>
        <div className="p-6 border rounded-lg">
          <h2 className="text-xl font-semibold mb-2">Total BPT Burned</h2>
          <p className="text-3xl font-bold">
            {data?.totalBptBurned.toLocaleString() ?? 0}
          </p>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Value (NGN)</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {data?.burnHistory.map((event) => (
                <tr key={event.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">{event.valueNgn.toLocaleString()}</td>

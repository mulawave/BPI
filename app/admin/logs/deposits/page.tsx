...
import { MdSearch, MdRefresh, MdDownload, MdCallMade } from "react-icons/md";
...
export default function DepositLogsPage() {
  ...
  const { data, isLoading, refetch } = api.admin.getDepositLogs.useQuery({
    page,
    pageSize,
  ...
  const successfulCount = transactionData.filter(t => 
    ['completed', 'success', 'successful'].includes(t.status)
  ).length;
  const pendingCount = transactionData.filter(t => t.status === 'pending').length;
  const totalAmountValue = transactionData
    .filter(t => ['completed', 'success', 'successful'].includes(t.status))
    ...
    <div className="space-y-6 pb-12">
      ...
      >
        ...
        <div className="relative overflow-hidden rounded-2xl border border-border bg-gradient-to-br from-card via-[hsl(var(--muted))] to-card p-8 shadow-xl">
          <div className="absolute -top-20 -right-20 h-40 w-40 rounded-full bg-gradient-to-br from-green-500 to-blue-600 opacity-10 blur-2xl" />
          <div className="relative">
            <div className="flex items-center gap-3 mb-2">
              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-green-500 to-blue-600 shadow-lg">
                <MdCallMade className="h-7 w-7 text-white" />
              </div>
              <div>
                <h1 className="text-3xl font-bold">Deposit Logs</h1>
                <p className="text-muted-foreground">View all deposit and funding transactions</p>
              </div>
            </div>
          </div>
        </div>
                  ...
                  <tr key={transaction.id} className="hover:bg-muted/50">
                    ...
                    <td className="px-4 py-3 text-sm">
                      <div>
                        ...
                        <div className="text-muted-foreground text-xs">{transaction.User?.email || ""}</div>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-sm">

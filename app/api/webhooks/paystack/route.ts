        ...
        if (fallbackPending?.transactionType) {
          ...
          if ((recoveredPurpose === 'MEMBERSHIP' || recoveredPurpose === 'MEMBERSHIP_PAYMENT') && recoveredPackageId && recoveredUserId) {
            ...
            if (claim.status === 'claimed') {
              if (await verifyPaymentAmount(claim.paymentId, amount / 100, reference, 'EMPOWERMENT')) {
                try {
                  await prisma.$transaction([
                    ...
                    prisma.pendingPayment.update({
                      ...
                      data: {
                        status: 'approved',
                        reviewedAt: new Date(),
                        reviewNotes: 'Auto-approved via Paystack webhook (empowerment purpose recovered from PendingPayment record)',
                      },
                    }),
                  ]);
                  ...
                  console.log('✅ [PAYSTACK-WEBHOOK] Empowerment processed via fallback recovery');
                } catch (error) {
                  await markPaymentNeedsReview(claim.paymentId, `Paystack fallback empowerment failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
                  console.error('❌ [PAYSTACK-WEBHOOK] Fallback empowerment failed:', error);
                }
              }
            }
              ...
              if (await verifyPaymentAmount(claim.paymentId, amount / 100, reference, 'STORE_PURCHASE')) {
                try {
                  ...
                  if (order && order.userId === recoveredUserId && order.status === 'PENDING') {
                    ...
                    await markPaymentApproved(claim.paymentId, 'Auto-approved via Paystack webhook (store purchase recovered from PendingPayment record)');
                    console.log('✅ [PAYSTACK-WEBHOOK] Store purchase processed via fallback recovery');
                  } else {
                  ...
                  await markPaymentNeedsReview(claim.paymentId, `Paystack fallback store purchase failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
                  console.error('❌ [PAYSTACK-WEBHOOK] Fallback store purchase failed:', error);
                }
              }
            }
          } else {
            ...
            await prisma.pendingPayment.updateMany({
              where: { gatewayReference: reference, status: { in: ['pending', 'processing'] } },
              data: {
                reviewNotes: `Paystack webhook: metadata purpose missing, recovered transactionType=${recoveredPurpose} but required fields incomplete.`,
              },
            });
          }
        } else {

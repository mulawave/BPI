...
<title>BeepAgro Palliative Initiative | Youtube Channels</title>
      ...
      <div class="row"> 
        ...
        <div class="col"> 
          ...
          <div class="page-title-container mb-3">
            <div class="row">
              <div class="col mb-2">
                <h1 class="mb-2 pb-0 display-4" id="title">Verified Youtube Channels</h1>
          ...
          <div class="row mb-5">
              <div class="col-xl-4 col-sm-12">
                  <div class="card h-100">
                    <div class="card-body">
                        <h5 class="mb-5 text-success">BPI YOUTUBE EARNINGS</h5>
                        <h1>
                        <?php
              ...
              <div class="col-xl-8 col-sm-12">
                  <div class="card h-100">
                    <div class="card-body">
                        <h5 class="text-success">RULES</h5>
                        <ul>
                            <li>Do not unsubscribe after earnings have been paid.</li>
                            <li>Do not attempt to click multiple subscribe buttons at once</li>
                            <li>Please note that violating any of the rules will get you banned automatically from the program!</li>
                        </ul>
                    </div>
                  </div>
              </div>
          ...
          <div class="row">
                <div class="col-12 mb-5">
                  <h2 class="small-title mb-3">Verified Channels</h2>
           <?php if($user_details->youtube_banned == 1){ ?>
                <div class="card">
                    <div class="card-body">
                        <h2>You are banned from this service for attempting to defraud the system!!</h2>
                    </div>
                </div>
            <?php }else{ ?>
                <div class="row">
                    <?php foreach ($channels as $channel): 
                       $subscription = $this->generic_model->getInfoCondition('channel_subscriptions','user_id', $user_details->id,array('channel_id' => $channel->id));
                       $provider = $this->generic_model->getInfo('youtube_provider','user_id',$channel->user_id);
                       $provider_balance = $provider->balance;
                       $channelID = $this->generic_model->extract_channel_id($channel->channel_url);
                       ?>
                       <?php if($provider_balance > 0): ?>
                        <div class="col-xl-3 mb-5">
                        <div class="card h-100">
                         <div class="card-body">
                             <div class="mb-3">
                              <h5><?php echo $channel->channel_name; ?></h5>
                             </div>
                            <div class="mb-3">
                            <img src="<?php echo $channel->channel_logo;?>" alt="youtube channel" width="100%">
                             </div>
                             <div class="text-center">   
                        <?php if (!empty($subscription)): ?>
                              <?php if($subscription->status == 'pending'):?> 
                                        <!-- Subscription Form -->
                                 <button id="sub-button" type="button" class="btn btn-danger btn-lg">Subscribe</button>
                                 <input type="hidden" id="channel_url" value="<?php echo $channel->channel_link; ?>">
                                 <div style="display:none" id="claim-div">
                                    <form id="subscribe-form" action="<?php echo base_url('channels/claim'); ?>" method="post">
                                        <input type="hidden" name="channel_id" value="<?php echo $channel->id; ?>">
                                        <button type="submit" class="btn btn-success btn-lg" id="subscribe-button">Claim</button>
                                    </form>
                                </div>
                              <?php else: ?>
                                        <p>Paid</p>
                              <?php endif; ?>
                        <?php else: ?>
                            <?php if($channel->user_id != $user_details->id):?> 
                                  <form id="subscribe-form" action="<?php echo base_url('channels/subscribe'); ?>" method="post">
                                        <input type="hidden" name="channel_id" value="<?php echo $channel->id; ?>">
                                        <button type="submit" class="btn btn-danger btn-lg" id="view-button">Process</button>
                                    </form>
                            <?php else: ?>
                                    <a href="#" data-bs-toggle="modal" data-bs-target="#stats-modal" class="btn btn-info btn-lg mt-3" id="upgrade-button">View Stats</a>
                                    <!-- implement upgrade option here -->
                                    <?php if($provider->youtube_plan < 3): ?>
                                    <!-- show upgrade modal -->
                                        <a href="#" data-bs-toggle="modal" data-bs-target="#upgrade-modal" class="btn btn-success btn-lg mt-3" id="upgrade-button"> Upgrade Plan</a>
                                    <?php else: ?>
                                    <p>Channel Owner</p>
                                    <?php endif; ?>
                            <?php endif; ?>
...
<div class="modal fade" id="stats-modal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" style="display: none;" aria-hidden="true">
<div class="modal-dialog">
<div class="modal-content">
<div class="modal-header">
<h5 class="modal-title" id="staticBackdropLabel">Promotion Statistics</h5>
<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
</div>
<div class="modal-body"> 
<div class="card">
    <?php 
        $provider_owner = $this->generic_model->getInfo('youtube_provider', 'user_id', $user_details->id);
        $ytPlan = $this->generic_model->getInfo('youtube_plans', 'id', $provider_owner->youtube_plan); ?>
    <div class="card-body">
        <div>
            <h3>Current Plan</h3>
            Name: <?php echo $ytPlan->name; ?><br>
            Amount: <?php echo $this->generic_model->getInfo('currency_management','id',$user_details->default_currency)->symbol; ?><?php echo $this->generic_model->convert_currency($user_details->default_currency,$ytPlan->amount);?><br>
            Quota: <?php echo $ytPlan->total_sub; ?> Subscribers <br>
            Quota Balance: <?php echo $provider_owner->balance; ?>
        </div>
    </div>
</div>
</div>
...
<div class="modal fade" id="upgrade-modal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" style="display: none;" aria-hidden="true">
<div class="modal-dialog">
<div class="modal-content">
<div class="modal-header">
<h5 class="modal-title" id="staticBackdropLabel">Youtube Promotion Upgrade</h5>
<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
</div>
<div class="modal-body"> 
    <?php $provider_upgrade = $this->generic_model->getInfo('youtube_provider', 'user_id', $user_details->id); ?>
    <form id="upgrade-form" action="<?php echo base_url('channels/upgrade'); ?>" method="post">
        <input type="hidden" name="channel_id" value="<?php echo $channel->id; ?>">
        <input type="hidden" name="youtube_plan" value="<?php echo $provider->youtube_plan;?>">
        <select required name="upgrade_plan" class="form-control mb-3 filled">
            <option value="">Select Upgrade Plan</option>
            <?php if($provider_upgrade->youtube_plan < 2): ?>
            <option value="2">
                Intermediate Plan (
                <?php echo $this->generic_model->getInfo('currency_management','id',$user_details->default_currency)->symbol; ?>
                <?php echo $this->generic_model->convert_currency($user_details->default_currency,62500);?> | 500 Subscribers)
                </option>
            <option value="3">
                Advanced Plan (
                <?php echo $this->generic_model->getInfo('currency_management','id',$user_details->default_currency)->symbol; ?>
                <?php echo $this->generic_model->convert_currency($user_details->default_currency,100000);?> | 1000 Subscribers)
                </option>
            <?php else: ?>
            <option value="3">
                Advanced Plan (
        ...
        <button type="submit" class="btn btn-success btn-lg" id="view-button">Upgrade</button>
    </form>
</div>
<div class="modal-footer">
...
<script>
    ...
    $('#sub-button').click(function() {
        // Get the channel URL from the hidden input field
        var channelUrl = $('#channel_url').val();

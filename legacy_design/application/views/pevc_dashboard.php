...
<title>BeepAgro Palliative Initiative | Community Support Dashboard</title>
  ...
  <main>
    <div class="container">
      <div class="row"> 
          ...
          <ul class="sw-25 side-menu mb-0 primary" id="menuSide">
            ...
            <li> <a href="#" data-bs-target="#dashboard"> <i data-acorn-icon="home" class="icon" data-acorn-size="18"></i> <span class="label">Member Dashboard</span> </a>
             ...
             <li> <a href="#" data-bs-target="#dashboard"> <i data-acorn-icon="home" class="icon" data-acorn-size="18"></i> <span class="label">Community Support</span> </a>
              <ul>
                <li> <a class="active" href="#"> <i data-acorn-icon="navigate-diagonal" class="icon d-none" data-acorn-size="18"></i> <span class="label">CS Dashboard</span> </a> </li>
              </ul>
            </li>
        ...
        <div class="col"> 
          ...
          <div class="page-title-container mb-3">
            <div class="row">
              <div class="col mb-2">
                <h1 class="mb-2 pb-0 display-4" id="title"> Community Support Dashboard</h1>
      ...
      <div class="col-lg-4 mb-4">
        <h2 class="mb-3">Support Requests (Global)</h2>
        <div class="card h-100 mb-4">
          <div class="card-body">
            <?php if (!empty($global_requests)) : ?>
              <ul class="list-group">
                <?php foreach ($global_requests as $req):
                  $expiry = date('Y-m-d H:i:s', strtotime($req->created_at . ' + ' . $req->timer . ' hours'));

                ?>
                  <li class="list-group-item">
                    <strong>Purpose:</strong> <?php echo $req->purpose; ?><br>
                    <strong>Amount:</strong> &#8358;<?php echo number_format($req->amount_requested); ?><br>
                    <?php if($req->user_id == $user_details->id){
                        echo 'Active';
                    }else{?>
                    <a href="<?php echo base_url('cs_donate/' . $req->id); ?>" class="btn btn-sm btn-primary mt-2">Donate</a>
                    <?php } ?>
                  </li>
                <?php endforeach; ?>
              </ul>
            <?php else : ?>
              <p>No approved global requests available.</p>
            <?php endif; ?>
          </div>
        </div>
      </div>
      <div class="col-lg-4 mb-4">
        <h2 class="mb-3">Support Requests (National)</h2>
        <div class="card h-100 mb-4">
          <div class="card-body">
            <?php if (!empty($national_requests)) : ?>
              <ul class="list-group">
                <?php foreach ($national_requests as $nreq): 
                    $recipient_nationality = $this->generic_model->getInfo('users','id', $nreq->user_id )->country;
                    if($recipient_nationality == $user_details->country) {
                     ?>
                      <li class="list-group-item">
                        <strong>Purpose:</strong> <?php echo $nreq->purpose; ?><br>
                        <strong>Amount:</strong> &#8358;<?php echo number_format($nreq->amount_requested); ?><br>
                        <a href="<?php echo base_url('donate/' . $nreq->id); ?>" class="btn btn-sm btn-primary mt-2">Donate</a>
                      </li>
                 <?php } ?>
                <?php endforeach; ?>
              </ul>
            <?php else : ?>
              <p>No approved national requests available.</p>
            <?php endif; ?>
          </div>
        </div>
      </div>
            ...
            <?php if (!empty($leaderboard)) : ?>
              <ol>
                <?php foreach ($leaderboard as $entry): ?>
                  <li>
                    <?php 
                      $donor = $this->generic_model->get_user_by_id($entry->donor_id);
                      echo 'Donor BPI-CS-#' . $entry->donor_id;
                    ?>
                    - &#8358;<?php echo number_format($entry->total_donated); ?>
                  </li>
                <?php endforeach; ?>
              </ol>
            <?php else : ?>
      ...
      <div class="col-md-6">
        <div class="card h-100">
          <div class="card-body">
            ...
            <p class="card-text">
              <strong>Total Requests:</strong> <?php echo $summary->total_requests ?? 0; ?><br>
              <strong>Total Amount Requested:</strong> &#8358;<?php echo number_format($summary->total_requested ?? 0); ?><br>
              <strong>Total Received:</strong> &#8358;<?php echo number_format($received->total_received ?? 0); ?>
            </p>
          </div>
        </div>
      </div>
       <div class="col-md-6">
        <div class="card h-100">
          <div class="card-body">
            ...
            <p class="card-text">
             <a href="<?php echo base_url('apply_for_support'); ?>" class="btn btn-lg btn-round btn-success">Apply For Support</a>
            </p>
            <?php }else{  ?>
            <p class="card-text">
             <?php echo $eligibility_remark; ?>
            </p>
    ...
    <div class="row mb-4">
      <div class="col-12">
        <div class="card">
          <div class="card-body">
            <h5 class="card-title">My Support Applications</h5>
              ...
              <table class="table table-bordered">
                ...
                <tbody>
                  <?php if (!empty($my_requests)): ?>
                    <?php foreach ($my_requests as $mreq): ?>
                      <tr>
                        <td><?php echo date('M d, Y', strtotime($mreq->created_at)); ?></td>
                        <td>&#8358;<?php echo number_format($mreq->amount_requested); ?></td>
                        <td><?php echo htmlspecialchars($mreq->purpose); ?></td>
                        <td><span class="badge bg-<?php echo ($mreq->status == 'approved') ? 'success' : (($mreq->status == 'pending') ? 'warning' : 'secondary'); ?>"><?php echo ucfirst($mreq->status); ?></span></td>
                      </tr>
                    <?php endforeach; ?>
                  <?php else: ?>
                ...
                <tbody>
                  <?php if (!empty($donations)): ?>
                    <?php foreach ($donations as $don): ?>
                      <?php $recipient = $this->generic_model->get_user_by_id($don->recipient_id); ?>
                      <tr>
                        <td><?php echo date('M d, Y', strtotime($don->donated_at)); ?></td>
                        <td>Support Request #<?php echo $don->support_request_id; //$recipient ? $recipient->firstname . ' ' . $recipient->lastname : 'User ID ' . $don->recipient_id; ?></td>
                        <td>&#8358;<?php echo number_format($don->amount); ?></td>
                      </tr>
                    <?php endforeach; ?>
                  <?php else: ?>
                    <tr><td colspan="3">No donations made yet.</td></tr>
                  <?php endif; ?>
                </tbody>
              </table>
            </div>

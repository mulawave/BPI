<?php
  ...
  $date = (!empty($is_ticket) && $is_ticket->status == 'claimed') ? $is_ticket->date_claimed : '';
?>
<!DOCTYPE html>
...
<title>BPI Solar Energy Assessment Tool</title>
    ...
    <div class="nav-content d-flex">
      ...
      <div class="user-container d-flex"> 
        <a href="#" class="d-flex user position-relative" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> 
          ...
          <div class="name"><?php echo htmlspecialchars($user_details->firstname . ' ' . $user_details->lastname); ?></div>
        </a>
                  ...
                  <a href="<?php echo base_url('notifications');?>">
                    <p class="text-primary">
                      ...
                      <em class="mt-2 text-success">
                        Received on: <?php echo htmlspecialchars($notification['created_at']); ?>
                      </em> 
                    </p>
                  </a> 
                </div>
      ...
      <div class="row"> 
        ...
        <div class="col-auto d-none d-lg-flex">
          <ul class="sw-25 side-menu mb-0 primary" id="menuSide">
            ...
            <li> 
              <a href="#" data-bs-target="#tools"> 
                <i data-acorn-icon="grid-1" class="icon" data-acorn-size="18"></i> 
                <span class="label">BPI Tools</span> 
              </a>
              <ul>
                <li> 
                  <a class="active" href="<?php echo base_url('solarassessment');?>"> 
                    <i data-acorn-icon="database" class="icon d-none" data-acorn-size="18"></i> 
                    <span class="label">Solar Assessment</span> 
                  </a> 
                </li>
              </ul>
            </li>
        ...
        <div class="col"> 
          ...
          <div class="page-title-container mb-3">
            <div class="row">
              <div class="col mb-2">
                <h1 class="mb-2 pb-0 display-4" id="title">BPI Solar Energy Assessment Tool</h1>
                <div class="text-muted mb-3 font-heading text-small">Empowering you with Real-Time Insights</div>
                <?php if ($user_details->user_type == 'user'): ?> 
                  <a href="<?php echo base_url('tickets/create'); ?>" class="btn mt-3 mb-3 btn-secondary btn-lg">
                    Create Ticket <sup><span class="badge bg-danger text-uppercase">New</span></sup>
                  </a> 
                <?php endif; ?>
                <hr class="bg-primary">
              </div>
              <div>
                ...
                <div class="alert alert-warning mb-3 mt-3 alert-dismissible fade show" role="alert">
                  ...
                  <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close">
                    <i data-acorn-icon="close-circle" class="text-white" data-acorn-size="18"></i>
                  </button>
                </div>
                <?php endif; ?>
                ...
                <div class="alert alert-danger mb-3 mt-3 alert-dismissible fade show" role="alert">
                  <strong>Form Errors!</strong> <?php echo validation_errors(); ?>
                  <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close">
                    <i data-acorn-icon="close-circle" class="text-white" data-acorn-size="18"></i>
                  </button>
                </div>
          ...
          <div class="row">
            <div class="col">
              ...
              <!-- Results Display -->
               <?php if (isset($total_load_kwh)): ?>
                  <div class="card mb-5">
                    <div class="card-body">
                      <h2 class="card-title mb-4">Assessment Results for <?php echo htmlspecialchars($client_name ?? 'Unknown'); ?></h2>
                      <div class="row">
                        <div class="col-md-6">
                          <h3 class="h5">System Requirements</h3>
                          <ul class="list-unstyled">
                            <li><strong>Total Load:</strong> <?php echo number_format($total_load_kwh ?? 0, 2); ?> kWh/day</li>
                            <li><strong>Inverter Capacity:</strong> <?php echo number_format($inverter_capacity ?? 0, 2); ?> kVA</li>
                            <li><strong>Battery Capacity:</strong> <?php echo number_format($battery_capacity ?? 0, 2); ?> Ah (<?php echo $number_of_batteries ?? 0; ?> units)</li>
                            <li><strong>Solar Panels:</strong> <?php echo number_format($panel_watts ?? 0, 2); ?> W (<?php echo $number_of_panels ?? 0; ?> panels)</li>
                            <li><strong>Estimated Runtime:</strong> <?php echo number_format($runtime_hours ?? 0, 2); ?> hours</li>
                          </ul>
                        </div>
                        <div class="col-md-6">
                          <h3 class="h5">Cost Summary (NGN)</h3>
                          <ul class="list-unstyled">
                            <li><strong>Inverter Cost:</strong> N<?php echo number_format($inverter_cost ?? 0, 2); ?></li>
                            <li><strong>Battery Cost:</strong> N<?php echo number_format($battery_cost ?? 0, 2); ?></li>
                            <li><strong>Solar Panel Cost:</strong> N<?php echo number_format($panel_cost ?? 0, 2); ?></li>
                            <li><strong>Additional Costs:</strong> N<?php echo number_format($additional_costs ?? 0, 2); ?></li>
                            <li><strong>Installation Cost:</strong> N<?php echo number_format($installation_cost ?? 0, 2); ?></li>
                            <li><strong>Assessment Fee:</strong> N<?php echo number_format($assessment_fee ?? 0, 2); ?></li>
                            <li><strong>Total Cost:</strong> N<?php echo number_format($total_cost ?? 0, 2); ?></li>
                          </ul>
                        </div>
                      </div>
                      <?php if (isset($assessment_id)): ?>
                        <a href="<?php echo base_url('solarassessment/download_pdf/' . $assessment_id); ?>" class="btn btn-primary mt-3">Download PDF</a>
                      <?php endif; ?>
                    </div>
      </div>
                <?php endif; ?>
                ...
                <div class="card">
      <div class="card-body">
        <form id="solarForm" action="<?php echo base_url('solarassessment/calculate'); ?>" method="post" enctype="multipart/form-data">
          <!-- Client Information -->
          <div class="mb-4">
            <h2 class="h4 mb-3">Client Information</h2>
            <div class="row g-3">
              <div class="col-md-4">
                <label class="form-label">Client Name</label>
                <input type="text" name="client_name" value="<?php echo set_value('client_name'); ?>" class="form-control" required>
              </div>
              <div class="col-md-4">
                <label class="form-label">Client Email</label>
                <input type="email" name="client_email" value="<?php echo set_value('client_email'); ?>" class="form-control" required>
              </div>
              <div class="col-md-4">
                <label class="form-label">SSC Code</label>
                <input type="text" name="ssc_code" value="<?php echo set_value('ssc_code'); ?>" class="form-control" required>
              </div>
            </div>
          </div>
          ...
          <!-- Region & State -->
          <div class="mb-4">
            <h2 class="h4 mb-3">Region & State</h2>
            <div class="row g-3">
              <div class="col-md-6">
                <label class="form-label">Installation Region</label>
                <select name="region" id="region" class="form-control" required>
                  <option value="">Select Region</option>
                  <?php foreach ($states as $region => $state_list): ?>
                    <option value="<?php echo htmlspecialchars($region); ?>" <?php echo set_select('region', $region); ?>><?php echo htmlspecialchars($region); ?></option>
                  <?php endforeach; ?>
                </select>
              </div>
              <div class="col-md-6">
          ...
          <!-- Appliance Data -->
          <div class="mb-4">
            <h2 class="h4 mb-3">Appliance Data</h2>
            <table id="applianceTable" class="table table-bordered">
              ...
              <tbody>
                <tr>
                  <td><input type="text" name="appliances[0][name]" class="form-control appliance-name" required>
                    <div class="invalid-feedback">Please enter appliance name.</div></td>
                  <td><input type="number" name="appliances[0][wattage]" class="form-control appliance-wattage" min="1" required>
                    <div class="invalid-feedback">Please enter valid wattage.</div></td>
                  <td>
                    <select name="appliances[0][unit]" class="form-control appliance-unit">
                      <option value="W">Watts</option>
                      <option value="HP">HP</option>
                    </select>
                  </td>
                  <td><input type="number" name="appliances[0][quantity]" class="form-control appliance-quantity" min="1" required>
                    <div class="invalid-feedback">Please enter valid quantity.</div></td>
                  <td><input type="number" name="appliances[0][hours]" class="form-control appliance-hours" min="0.1" step="0.1" required>
                    <div class="invalid-feedback">Please enter valid hours.</div></td>
                  <td><span class="remove-btn">×</span></td>
                </tr>
              </tbody>
            </table>
            <button type="button" id="addAppliance" class="btn btn-primary">Add Appliance</button>
          </div>

          <!-- Solar Use Duration -->
          <div class="mb-4">
            <h2 class="h4 mb-3">Solar Use Duration</h2>
            <label class="form-label">Daily Runtime (hours)</label>
            <input type="number" name="daily_runtime" value="<?php echo set_value('daily_runtime'); ?>" class="form-control" min="0.1" step="0.1" required>
            <div class="invalid-feedback">Please enter valid runtime hours.</div>
          </div>

          <!-- Component Selections -->
          <div class="mb-4">
            <h2 class="h4 mb-3">Component Selections</h2>
            <div class="row g-3">
              <div class="col-md-4">
                <label class="form-label">Inverter</label>
                <select name="inverter_id" class="form-control" required>
                  <?php if (isset($products['inverters']) && is_array($products['inverters'])): ?>
                    <?php foreach ($products['inverters'] as $id => $inverter): ?>
                      <option value="<?php echo htmlspecialchars($id); ?>" <?php echo set_select('inverter_id', $id); ?>>
                        <?php echo htmlspecialchars($inverter['name'] . ' (BPI: N' . number_format($inverter['bpi_price']) . ', Non-BPI: N' . number_format($inverter['non_bpi_price']) . ')'); ?>
                      </option>
                    <?php endforeach; ?>
                  <?php else: ?>
                    <option value="">No inverters available</option>
                  <?php endif; ?>
                </select>
              </div>
              <div class="col-md-4">
                <label class="form-label">Battery</label>
                <select name="battery_id" class="form-control" required>
                  <?php if (isset($products['batteries']) && is_array($products['batteries'])): ?>
                    <?php foreach ($products['batteries'] as $id => $battery): ?>
                      <option value="<?php echo htmlspecialchars($id); ?>" <?php echo set_select('battery_id', $id); ?>>
                        <?php echo htmlspecialchars($battery['name'] . ' (BPI: N' . number_format($battery['bpi_price']) . ', Non-BPI: N' . number_format($battery['non_bpi_price']) . ')'); ?>
                      </option>
                    <?php endforeach; ?>
                  <?php else: ?>
              ...
              <div class="col-md-4">
                ...
                <select name="panel_id" class="form-control" required>
                  <?php if (isset($products['panels']) && is_array($products['panels'])): ?>
                    <?php foreach ($products['panels'] as $id => $panel): ?>
                      <option value="<?php echo htmlspecialchars($id); ?>" <?php echo set_select('panel_id', $id); ?>>
                        <?php echo htmlspecialchars($panel['name'] . ' (BPI: N' . number_format($panel['bpi_price']) . ', Non-BPI: N' . number_format($panel['non_bpi_price']) . ')'); ?>
                      </option>
                    <?php endforeach; ?>
                  <?php else: ?>
                    <option value="">No panels available</option>
                  <?php endif; ?>
                </select>
              </div>
            </div>
          ...
          <div class="mb-4">
            <h2 class="h4 mb-3">Installation Options</h2>
            <label class="form-label">Installation Type</label>
            <select name="installation_option" class="form-control" required>
              <option value="Corporate" <?php echo set_select('installation_option', 'Corporate'); ?>>BPI Corporate Installation</option>
              <option value="Self" <?php echo set_select('installation_option', 'Self'); ?>>Self Installation</option>
            </select>
          </div>

          <!-- Installation Site Details -->
          <div class="mb-4">
            <h2 class="h4 mb-3">Installation Site Details</h2>
            <div class="row g-3">
              <div class="col-md-6">
                <label class="form-label">Installation Address</label>
                <textarea name="installation_address" class="form-control" required><?php echo set_value('installation_address'); ?></textarea>
              </div>
              <div class="col-md-6">
                <label class="form-label">Contact Person</label>
                <input type="text" name="contact_person" value="<?php echo set_value('contact_person'); ?>" class="form-control" required>
              </div>
              <div class="col-md-6">
                <label class="form-label">Contact Phone</label>
                <input type="text" name="contact_phone" value="<?php echo set_value('contact_phone'); ?>" class="form-control" required>
              </div>
              <div class="col-md-6">
                <label class="form-label">Preferred Installation Date</label>
                <input type="date" name="installation_date" value="<?php echo set_value('installation_date'); ?>" class="form-control" required>
              </div>
            </div>
          </div>
          ...
          <!-- Payment Proof -->
          <div class="mb-4">
            <h2 class="h4 mb-3">Payment Proof</h2>
            <label class="form-label">Upload Proof of Payment (JPG, PNG, PDF)</label>
            <input type="file" name="proof_of_payment" class="form-control" accept=".jpg,.png,.pdf" required>
          </div>
...
<div class="settings-buttons-container">
  <button type="button" class="btn settings-button btn-primary p-0" data-bs-toggle="modal" data-bs-target="#settings" id="settingsButton"> 
    <span class="d-inline-block no-delay" data-bs-delay="0" data-bs-offsetką0="0,3" data-bs-toggle="tooltip" data-bs-placement="left" title="Settings"> 
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header border-0 p-0">
        <button class="btn-close btn btn-icon btn-icon-only btn-foreground" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body ps-5 pe-5 pb-0 border-0">
        <input id="searchPagesInput" class="form-control form-control-lg borderless ps-0 pe-0 mb-1 auto-complete" type="text" autocomplete="off">
      </div>
      <div class="modal-footer border-top justify-content-start ps-5 pe-5 pb-3 pt-3 border-0"> 
        ...
        <span class="text-alternate d-inline-block m-0 me-3"> 
          <i data-acorn-icon="arrow-bottom-left" data-acorn-size="15" class="text-alternate align-middle"></i> 
...
<script>
    (function($) {
      $(document).ready(function() {
        ...
        try {
          const states = <?php echo json_encode($states, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT); ?> || {};
          $('#region').on('change', function() {
            const region = $(this).val();
            const stateSelect = $('#state');
            stateSelect.empty().append('<option value="">Select State</option>');
            if (region && states[region] && Array.isArray(states[region])) {
              states[region].forEach(state => {
                stateSelect.append(`<option value="${encodeURIComponent(state)}">${state}</option>`);
              });
            }
          });
        } catch (e) {
          console.error('Error initializing region/state dropdown:', e);
        ...
        $('#addAppliance').on('click', function() {
          const row = `
            <tr>
              <td><input type="text" name="appliances[${rowIndex}][name]" class="form-control appliance-name" required>
                <div class="invalid-feedback">Please enter appliance name.</div></td>
              <td><input type="number" name="appliances[${rowIndex}][wattage]" class="form-control appliance-wattage" min="1" required>
                <div class="invalid-feedback">Please enter valid wattage.</div></td>
              <td>
                <select name="appliances[${rowIndex}][unit]" class="form-control appliance-unit">
                  <option value="W">Watts</option>
                  <option value="HP">HP</option>
                </select>
              </td>
              <td><input type="number" name="appliances[${rowIndex}][quantity]" class="form-control appliance-quantity" min="1" required>
                <div class="invalid-feedback">Please enter valid quantity.</div></td>
              <td><input type="number" name="appliances[${rowIndex}][hours]" class="form-control appliance-hours" min="0.1" step="0.1" required>
                <div class="invalid-feedback">Please enter valid hours.</div></td>
              <td><span class="remove-btn">×</span></td>
            </tr>
          `;
          $('#applianceTable tbody').append(row);
          rowIndex++;
        });

        // Remove Appliance
        $(document).on('click', '.remove-btn', function() {
          if ($('#applianceTable tbody tr').length > 1) {
            $(this).closest('tr').remove();
          } else {
            alert('At least one appliance is required.');
          }
        });
        ...
        $('#solarForm').on('submit', function(e) {
          let valid = true;
          $('#applianceTable tbody tr').each(function() {
            const row = $(this);
            const name = row.find('.appliance-name').val().trim();
            const wattage = parseFloat(row.find('.appliance-wattage').val());
            const quantity = parseInt(row.find('.appliance-quantity').val());
            const hours = parseFloat(row.find('.appliance-hours').val());

            row.find('.appliance-name').toggleClass('is-invalid', !name);
            row.find('.appliance-wattage').toggleClass('is-invalid', !wattage || wattage <= 0);
            row.find('.appliance-quantity').toggleClass('is-invalid', !quantity || quantity <= 0);
            row.find('.appliance-hours').toggleClass('is-invalid', !hours || hours <= 0);

            if (!name || !wattage || wattage <= 0 || !quantity || quantity <= 0 || !hours || hours <= 0) {
              valid = false;
            }
          });
          ...
          if (!valid) {
            e.preventDefault();
            alert('Please fill out all appliance fields correctly.');
          }
        });
      });
    })(jQuery);

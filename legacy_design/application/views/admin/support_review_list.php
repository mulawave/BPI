      ...
      <div class="row"> 
        ...
        <div class="col-auto d-none d-lg-flex">
          <ul class="sw-25 side-menu mb-0 primary" id="menuSide">
            ...
            <li> <a href="<?php echo base_url('admin');?>" data-bs-target="#dashboard"> <i data-acorn-icon="home" class="icon" data-acorn-size="18"></i> <span class="label">User Section</span> </a>
              <ul>
				  <?php if($user_details->user_type == 'admin'){ ?>
				<li> 
					<a class="" href="#"> <i data-acorn-icon="home" class="icon d-none" data-acorn-size="18"></i> <span class="label">Admin Overview</span> </a> 
				  </li>
				  <li > 
					<a class="active" href="#"> <i data-acorn-icon="shield-check" class="icon d-none" data-acorn-size="18"></i> <span class="label">CS Requests</span> </a> 
				  </li>
				  <!-- <li > 
      ...
      <div class="card-body">
        ...
        <div class="table-responsive">
          <table class="table table-bordered">
            ...
            <tbody>
              <?php foreach ($pending_requests as $i => $request):
                $userInfo = $this->generic_model->getInfo('users','id',$request->user_id);
              ?>
              <tr>
                <td><?php echo $i + 1; ?></td>
                <td><?php echo $userInfo->firstname . ' ' . $userInfo->lastname; ?></td>
                <td>&#8358;<?php echo number_format($request->amount_requested,2); ?></td>
                <td><?php echo $request->purpose; ?></td>
                <td><span class="badge bg-info"><?php echo strtoupper($request->level); ?></span></td>
                <?php if($request->status == 'declined'){ ?>
                    <td><span class="badge bg-danger text-white"><?php echo ucfirst($request->status); ?></span></td>
                <?php }else{ ?>
                <td><span class="badge bg-warning text-dark"><?php echo ucfirst($request->status); ?></span></td>
                <?php } ?>
                <td><?php echo date('M d, Y', strtotime($request->created_at)); ?></td>
                <td>
                  <?php if ($request->media_url): ?>
                    <a href="<?php echo base_url($request->media_url); ?>" target="_blank">View</a>
                  <?php else: ?>
                    -
                  <?php endif; ?>
                </td>
                <td>
                <?php if($request->status == 'declined'){ ?>
                  <a href="<?php echo base_url('admin/delete_cs_request/'.$request->id); ?>" class="btn btn-sm btn-danger">DELETE</a>
                 <?php }else{ ?> 
                  <a href="<?php echo base_url('admin/approve_cs_request/'.$request->id); ?>" class="btn btn-sm btn-success">Approve</a>
                  <a href="<?php echo base_url('admin/reject_cs_request/'.$request->id); ?>" class="btn btn-sm btn-danger">Reject</a>
                 <?php } ?>
                </td>
              </tr>
              <?php endforeach; ?>

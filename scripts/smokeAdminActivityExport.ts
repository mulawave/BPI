/*
  Admin Activity Export runtime smoke:
  - Calls exportAuditLogsToCSV

  Usage:
    npm run smoke:admin-activity-export
*/
...
async function main() {
  const admin = await ensureSuperAdmin("admin+smoke-activity-export@example.com");
  const session = createSessionForUser(admin.id, "super_admin");
  const caller = adminRouter.createCaller({ session, prisma } as any);

  const csv = await caller.exportAuditLogsToCSV({});
  console.log("Audit export:", csv.filename, "rows:", csv.count);
  if (!csv.data || csv.data.split("\n").length < 2) throw new Error("Audit CSV unexpectedly small");
}

main()
  .catch((e) => {
    console.error("smokeAdminActivityExport failed:", e);
    process.exitCode = 1;
  })
  .finally(async () => {
